...
struct uncomplete {
    ...
    int read;                // 已经读取了多少个字节,-1表示只读取到了本数据包的一个字节数据
    int header;              // 应用层数据包头部信息，当新的应用层数据包只能读取首部的1个字节时，将其临时存放到这里
};
...
struct queue {
    ...
    struct netpack queue[QUEUESIZE];   // 如果传过来的包多于一个，就会暂存在这里,并向lua返回TYPE_MORE，lua就会从queue里读取
};
...
find_uncomplete(struct queue *q, int fd) { //从map中查找并移除
    if (q == NULL)
        return NULL;
    int h = hash_fd(fd);
...
filter_data_(lua_State *L, int fd, uint8_t *buffer, int size) {
    ...
    if (uc) { //上次数据包还没有读完
        // fill uncomplete
        if (uc->read < 0) { //如果 read 小于 0，说明上次只读取到了本数据包的一个字节数据
            // read size
            assert(uc->read == -1);
            int pack_size = *buffer; //读第二个字节的数据来得到数据包长度
        ...
        int need = uc->pack.size - uc->read; //这个数据包还有多少个字节需要读取
        if (size < need) {
        ...
        buffer += need;
        size -= need;
        if (size == 0) { //刚好能够读取到一个完整的数据包，压栈返回
        ...
        // more data
        push_data(L, fd, uc->pack.buffer, uc->pack.size, 0); //将刚才读到的完整的包放到queue中
        skynet_free(uc);
        push_more(L, fd, buffer, size);
        lua_pushvalue(L, lua_upvalueindex(TYPE_MORE));
        return 2;
    } else {
        if (size == 1) { //本次读取的数据长度为 1
            struct uncomplete *uc = save_uncomplete(L, fd);
            uc->read = -1;
            uc->header = *buffer;
            return 1;
