...
struct message_queue *skynet_mq_create(uint32_t handle) {
    ...
    // If the service init success, skynet_context_new will call skynet_mq_push to
    // push it to global queue.
...
uint32_t skynet_mq_handle(struct message_queue *q) { return q->handle; }
