...
static ATOM_SIZET _used_memory = 0;  //本进程内通过 skynet_malloc 分配的所有内存数量
static ATOM_SIZET _memory_block = 0; //本进程内通过 skynet_malloc 分配的内存块数，跟分配次数相等

//每个服务都有
struct mem_data {
    ATOM_ULONG handle;
    ATOM_SIZET allocated; //保存该服务已分配的内存字节数
};

//每次分配内存的时候都会在分配出的内存块后面加上这样一块后缀数据
struct mem_cookie {
    uint32_t handle;
#ifdef MEMORY_CHECK
    uint32_t dogtag;
...
skynet_malloc(size_t size) {
    void *ptr = je_malloc(size + PREFIX_SIZE); //分配一块需求长度 + 后缀长度的内存
    if (!ptr)
        malloc_oom(size);
    return fill_prefix(ptr); //为内存块填充后缀，填充完毕以后修改上面提到的各种数据计数
}
